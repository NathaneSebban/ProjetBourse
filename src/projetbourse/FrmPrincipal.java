...
public class FrmPrincipal extends javax.swing.JFrame {
    ...
    private void tblTradersMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblTradersMouseClicked
        ...
        String nomTrader;
        nomTrader = tblTraders.getValueAt(tblTraders.getSelectedRow(), 1).toString();
        for(Trader trad : mesTraders)
        {
           if(trad.getNomTrader().compareTo(nomTrader)==0)
           {
               for(Action act : trad.getLesActions())
               {
                   v = new Vector();
                   v.add(act.getNumeroAction());
                   v.add(act.getNomAction());
                   v.add(act.getValeurActuelle());
                   v.add(act.getPrixAchat());
                   v.add(act.getQuantiteAchete());
                   dtmActions.addRow(v);
               }
           }
        }
        ...
        for(Trader trad : mesTraders)
        {
           if(trad.getNomTrader().compareTo(nomTrader)==0)
           {
               for(Action act : trad.getLesActions())
               {
                   montantPortefeuille = montantPortefeuille + (act.getValeurActuelle() * act.getQuantiteAchete() - (act.getPrixAchat() * act.getQuantiteAchete()));
               }
           }
        }
    ...
    private void tblActionsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblActionsMouseClicked
        ...
        String nomAction;
        nomAction = tblActions.getValueAt(tblActions.getSelectedRow(), 1).toString();
        double gain = 0;
        double resultat = 0;
        ...
        for(Trader trad : mesTraders)
        {
               for(Action act : trad.getLesActions())
               {
                   if(act.getNomAction().compareTo(nomAction)==0)
                   {   
                   if(act.getValeurActuelle() > act.getPrixAchat())
                   {    
                       resultat = act.getValeurActuelle() - act.getPrixAchat();
                       gain = resultat * act.getQuantiteAchete();
